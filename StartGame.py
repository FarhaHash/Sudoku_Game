...
INVALID_TEXT = """Invalid difficulty level selected"""
ABOUT_TEXT = """                                                            
...
class StartGame(object):
    ...
    def game(self, boards_file,name):
        ...
        game = SudokuGame.SudokuGame(boards_file)
        game.start()
        SudokuGUI.SudokuGUI(self.root, game)
    ...
    def __submit(self):
        if self.e2.get().lower() == "Easy".lower():
            ...
            toplevel = Toplevel()
            label1 = Label(toplevel, text=INVALID_TEXT, height=0, width=50)
            label1.pack()
